    ...
    def convert(self):
        if (self.sender().column == self.column_last
            ...
            if self.sender().times == 0:
                self.sender().setText(self.Board.names[self.sender().line][self.sender().column])
                s = self.sender().text()
                self.sender().times = 1
                if s != '':
                    if s[0] == '+':
...
class BonusWindow(QWidget):
    def __init__(self):
        ...
        self.info = QLabel(self)
        self.info.setText('Бонусы прдеставляет собой эффект, происходящий в случае '
...
class GameOver(QDialog):
    def __init__(self, move_count, score_count, record_id):
        ...
        self.res = 5 * int(score_count) - 3 * int(move_count)
